...
LDFLAGS += -X "github.com/moooofly/dms-detector/pkg/version.ClientVersion=$(shell cat VERSION)"
LDFLAGS += -X "github.com/moooofly/dms-detector/pkg/version.GoVersion=$(shell go version)"
LDFLAGS += -X "github.com/moooofly/dms-detector/pkg/version.UTCBuildTime=$(shell date -u '+%Y-%m-%d %I:%M:%S')"
LDFLAGS += -X "github.com/moooofly/dms-detector/pkg/version.GitBranch=$(shell git rev-parse --abbrev-ref HEAD)"
LDFLAGS += -X "github.com/moooofly/dms-detector/pkg/version.GitTag=$(shell git describe --tags)"
LDFLAGS += -X "github.com/moooofly/dms-detector/pkg/version.GitHash=$(shell git rev-parse HEAD)"
